...
def processArguments(args):
    ...
    return config
#---------------------------------------------------------------------------
def infer(model, tokenizer, prompt, deepSpeed=False, dsModel=None):
    ...
    for trainInd, trainFile in enumerate(tqdm(config.trainFiles, desc="Train File")):
        ...
        for testInd, testFile in enumerate(tqdm(config.testFiles, desc="Test File")):
            ...
            for testInstance in tqdm(testData, desc="Test Instance"):
                ...
                genText = infer(model, tokenizer, finalPrompt, config.deepSpeed, dsModel)
                ...
                if prediction.lower() == testInstance["answerKey"].lower():
                    ...
                    if config.rationalize:
                        ...
                        genText = infer(model, tokenizer, rationalizedFinalPrompt, config.deepSpeed, dsModel)
                        extractedAnswer = extractAnswer(genText, config.direct)
                        if extractedAnswer == None:
                            continue
                        prediction = extractedAnswer["answer"]
                ...
                logging.info("*"*50)
            logging.info("Accuracy: {:0.2f} ({}/{})".format((accuracyScore/len(testData)), accuracyScore, len(testData)))
            if config.rationalize:
                logging.info("Rationalization Accuracy: {:0.2f} ({}/{})".format((rationalizedAccuracyScore/(len(rationalizedCorrectPreds)+len(rationalizedWrongPreds))), rationalizedAccuracyScore, (len(rationalizedCorrectPreds)+len(rationalizedWrongPreds))))
            if not config.outPath.endswith("/"):
                config.outPath += "/"
            with open(f'{config.outPath}{trainFile.split("/")[-1].split(".")[0]}_{testFile.split("/")[-1].split(".")[0]}_{config.dataset}.json', 'w') as fout:
                json.dump(outputs , fout)
            with open(f'{config.outPath}{trainFile.split("/")[-1].split(".")[0]}_{testFile.split("/")[-1].split(".")[0]}_{config.dataset}_correct.json', 'w') as fout:
                json.dump(correctPreds , fout)
            with open(f'{config.outPath}{trainFile.split("/")[-1].split(".")[0]}_{testFile.split("/")[-1].split(".")[0]}_{config.dataset}_wrong.json', 'w') as fout:
                json.dump(wrongPreds , fout)
            with open(f'{config.outPath}{trainFile.split("/")[-1].split(".")[0]}_{testFile.split("/")[-1].split(".")[0]}_{config.dataset}_rationalizedCorrect.json', 'w') as fout:
                json.dump(rationalizedCorrectPreds , fout)
            with open(f'{config.outPath}{trainFile.split("/")[-1].split(".")[0]}_{testFile.split("/")[-1].split(".")[0]}_{config.dataset}_rationalizedWrong.json', 'w') as fout:
